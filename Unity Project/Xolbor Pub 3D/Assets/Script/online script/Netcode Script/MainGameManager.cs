...
public class MainGameManager : NetworkBehaviour
{
    ...
    /*
    void OnGUI()    //GUI buttons control functional buttons created by script
    {
        GUILayout.BeginArea(new Rect(10, 10, 300, 300));                                    //create area for buttons
        if (!NetworkManager.Singleton.IsClient && !NetworkManager.Singleton.IsServer)       //is current player not the client? and...
