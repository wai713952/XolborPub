...
public class test_control : NetworkBehaviour
{
    ...
    private void Update()
    {
        ...
        if ((Input.GetKeyDown(KeyCode.Alpha2) || Input.GetKeyDown(KeyCode.Keypad2)) && Input.GetKey(KeyCode.Z) && canSwitchScene == true)
        {
            if (SceneManager.GetActiveScene().name == ("scene2_login")) { return; }
            canSwitchScene = false;
            NetworkManager.Singleton.Shutdown();
            SceneManager.LoadScene("scene2_login");
        }
